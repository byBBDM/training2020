...
namespace Task4
{
    public class Program
    {
        public static bool FindMinMax(int[][] array, out int min, out int max)
        {
            if (array == null)
            {
                throw new ArgumentNullException("Передано значение null в метод FindMinMax() в качестве параметра!");
            }
            min = FindMin(array);
            max = FindMax(array);
        ...
        public static bool IsValue(int[][] jagged)
        {
            for (int i = 0; i < jagged.Length; ++i)
            {
                ...
                {
                    if (jagged[i][j].GetType() == typeof(Int32))
                    {
                        return true;
                    }
                }
